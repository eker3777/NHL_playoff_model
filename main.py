...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("nhl_prediction.log"), logging.StreamHandler()],
)
...
def load_data(season: int = None) -> Dict[str, Any]:
    ...
    standings_df = (
        data_loader.process_standings_data(standings_raw, season_str)
        if standings_raw
        else None
    )
...
def generate_sample_matchups(team_data, model_predictor):
    ...
    if "points" in team_data.columns:
        ...
        matchups = [
            ...
            (1, 6),
            (2, 5),
            (3, 4),  # Eastern Conference
            (8, 15),
            (9, 14),
            (10, 13),
            (11, 12),  # Western Conference
        ]
        ...
        for i, (top_idx, bottom_idx) in enumerate(matchups):
            if top_idx < len(sorted_teams) and bottom_idx < len(sorted_teams):
                ...
                matchup_df = model_predictor.create_matchup_data(
                    ...
                    {
                        "teamAbbrev": bottom_seed.get(
                            "teamAbbrev", f"Team{bottom_idx+1}"
                        )
                    },
                )

                # Generate prediction
                ensemble_prob, lr_prob, xgb_prob = (
                    model_predictor.predict_series_winner(matchup_df)
                )
                ...
                prediction = {
                    ...
                    "bottom_seed_abbrev": bottom_seed.get(
                        "teamAbbrev", f"Team{bottom_idx+1}"
                    ),
                ...
                logger.debug(
                    f"Prediction: {prediction['top_seed_abbrev']} vs {prediction['bottom_seed_abbrev']} - {ensemble_prob:.1%}"
                )
...
def generate_visualizations(team_data, standings_data, predictions, reports_dir):
    ...
    if team_data is not None and not team_data.empty:
        chart_paths["season_performance"] = visualizer.create_season_performance_chart(
            team_data
        )

    # Generate playoff bracket chart
    if predictions:
        chart_paths["playoff_bracket"] = visualizer.create_playoff_bracket_chart(
            predictions
        )
    ...
    if predictions:
        ...
        chart_paths["prediction_confidence"] = (
            visualizer.create_prediction_confidence_chart(predictions_df)
        )
...
def main(season: int = None):
    ...
    try:
        ...
        predictions, model_info = generate_predictions(
            data_dict["team_data"], models_folder
        )
        ...
        report_path = generate_report(
            data_dict, predictions, model_info, chart_paths, reports_dir
        )
