...
from streamlit_app.utils.cache_manager import load_cached_simulation_results
...
def display_simulation_results(model_data=None):
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Championship Odds", "Team Odds", "Round-by-Round", "Potential Matchups"])
    ...
    with tab2:
        st.subheader("Team Playoff Advancement Probabilities")
        
        # Format all probability columns for display
        display_df = results_df.copy()
        for col in ['round_1', 'round_2', 'conf_final', 'final', 'champion']:
            display_df[col] = (display_df[col] * 100).round(1).astype(str) + '%'
        
        # Round average games played
        display_df['avg_games'] = display_df['avg_games_played'].round(1)
        
        # Sort by champion probability
        sort_order = results_df['champion'].values
        display_df['sort_key'] = sort_order
        display_df = display_df.sort_values('sort_key', ascending=False).drop('sort_key', axis=1)
        ...
        st.dataframe(display_df[display_cols.keys()].rename(columns=display_cols), use_container_width=True)
        
        # Create interactive chart for top teams
        st.subheader("Round-by-Round Advancement Probabilities")
        
        # Get top teams for the chart
        top_teams = results_df.sort_values('champion', ascending=False).head(10)
        
        # Prepare data for stacked chart
        rounds = ['round_1', 'round_2', 'conf_final', 'final', 'champion']
        round_names = ['First Round', 'Second Round', 'Conference Finals', 'Stanley Cup Final', 'Champion']
        
        # Melt the dataframe to get it in the right format for Altair
        chart_data = top_teams[['teamName'] + rounds].melt(
            id_vars=['teamName'],
            value_vars=rounds,
            var_name='round',
        ...
        # Map round codes to human-readable names
        round_map = dict(zip(rounds, round_names))
        chart_data['round_name'] = chart_data['round'].map(round_map)
        
        # Create stacked bar chart using Altair
        stacked_chart = alt.Chart(chart_data).mark_bar().encode(
            x=alt.X('probability_pct:Q', axis=alt.Axis(format='~r'), title='Probability (%)'),
            y=alt.Y('teamName:N', title='Team'),
            color=alt.Color('round_name:N', 
                           scale=alt.Scale(domain=round_names, 
                                          range=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']),
                           legend=alt.Legend(title='Playoff Round')),
            tooltip=['teamName', 'round_name', alt.Tooltip('probability_pct:Q', format='.1f', title='Probability %')]
        ).properties(
            title='Round-by-Round Advancement Probabilities',
            height=400
        )
...
def app():
    """Main entry point for the simulation results page"""
    # Call the display function
    display_simulation_results()

def show_simulation_results():
    """Entry point for simulation results page called from app.py"""
    app()
